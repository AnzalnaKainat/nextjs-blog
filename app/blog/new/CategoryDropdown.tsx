...
type Props = {
  list: SelectOption[];
  selected: number | null;
  setSelected: (select: number | null) => void;
}
  ...
  return (
    <div className='my-6 relative'>
      <button
        type='button'
        onClick={toggleOpen}
        className='flex justify-between items-center gap-2 px-3 py-3 border border-[#0f277b] text-[#0f277b] rounded-md'
      >
        {currSelection.name}
      ...
      {open && (
        <div className='absolute mt-2 bg-white text-[#0f277b] rounded-md border border-[#0f277b] z-50'>
          {props.list.map((item) => (
            <div
              key={item.id}
              className="px-3 py-2 hover:bg-[#0f277b] hover:text-white cursor-pointer"
